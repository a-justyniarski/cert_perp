...
class CertPrepTutoring:
	...
	_title_person_authorised: str = transform_polish_char_to_latex('Dyrektor NPDN Instytutu Educare')
	...
	def __init__(
		...
		self.certificate_no = kwargs.get('certificate_no') if kwargs.get('certificate_no') is not NaN \
			else certificate_no
	...
	def prepare_preamble(self) -> None:
		...
		packages = [
			...
			Package('csquotes', options=f"style={self.quotes_style}"),
			Package('babel'),
		]
	...
	def append_header(
			self, text: str = None, font_size: float = 50, upper: bool = True,
	...
	def append_subheader(
			self, text: str = None, font_size: float = 15, is_bold: bool = True,
		...
		if is_bold:
			...
			h2_block.append(insert_text_font_f(header2, h2_font_size))
	...
	def append_birthday(
		...
		if not text:
			text = self.birth_text.format(self.birthday, self.birthplace)
		name_font_size = str(font_size)
	...
	def append_training_header(
			self, text: str = None, font_size: float = 15, width: float = 20,
			vertical_pos: float = 12, horizontal_pos: float = 0,
	) -> None:
		if text is None:
			text = self.header_training
	...
	def append_training_date(
		...
		if not training_date_text:
			training_date_text_main = f'trwającej od {training_date_list[0]} r. do {training_date_list[-1]} r.'
			training_date_text_sub = f'w wymiarze ' \
				f'{int(training_duration) if training_duration.is_integer() else training_duration} ' \
				f'{"godziny" if training_duration == 1 else "godzin"}'

			training_date_block.append(insert_text_font_f(training_date_text_main, training_date_font_size))
			training_date_block.append(LineBreak())
			training_date_block.append(insert_text_font_f(training_date_text_sub, training_date_font_size))
		else:
	...
	def append_training_scope_header(
			self, text: str = None, upper_header: bool = True, font_size: float = 15,
			width: float = 20, vertical_pos: float = 17, horizontal_pos: float = 1.2,
	) -> None:
		if text is None:
			text = self.training_scope_header
	...
	def append_policy_statement(
			self, text: str = None, font_size: float = 10, width: float = 19,
			vertical_pos: float = 25, horizontal_pos: float = 1.2,
	) -> None:
		if text is None:
			text = self.policy_statement_text
	...
	def append_sign_space(
			self, title_person_authorised: str = None, person_authorised: str = None,
			font_size: float = 10, width: float = 20, vertical_pos: float = 28, horizontal_pos: float = 0,
	) -> None:
		if title_person_authorised is None:
			title_person_authorised = self.title_person_authorised
		if person_authorised is None:
			person_authorised = self.person_authorised
		...
		sign_space_block.append(insert_text_font_f('.'*60, sign_space_font_size))
	...
	def prepare_certificate_pdf(self):
		...
		# self.set_background()
		self.append_header()
		self.append_underline(vertical_pos=6.6)
		self.append_subheader(text='UKOŃCZENIA', font_size=12, is_bold=False, vertical_pos=7.1)
		self.append_training_name(vertical_pos=8.2, quotes=True)
		self.append_subheader(text='I STOPNIA', vertical_pos=9.5)
		self.append_training_date(vertical_pos=10.5)
		self.append_subheader(text='OTRZYMUJE', vertical_pos=12, is_bold=False)
		self.append_name(vertical_pos=13.2, font_size=30)
		self.append_birthday(vertical_pos=15)
		# self.append_training_header()
		self.append_training_scope_header()
		self.append_training_scope()
		self.append_policy_statement()
		self.append_sign_space(horizontal_pos=5)
